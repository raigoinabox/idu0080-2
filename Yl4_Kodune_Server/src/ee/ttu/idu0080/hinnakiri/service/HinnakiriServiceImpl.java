...
public class HinnakiriServiceImpl implements HinnakiriService {
	...
	public GetHinnakiriResponse getHinnakiri(java.lang.String parameters) throws HinnakiriNumberFormatException,
			HinnakiriNumberNegativeException, HinnakiriNumberZeroException, HinnakiriNumberDecimalException {
		...
		if (maximumPrice < 0) {
			throw new HinnakiriNumberNegativeException();
		}

		if (maximumPrice == 0) {
			throw new HinnakiriNumberZeroException();
		}

		int separatorIdx = parameters.indexOf('.');
		if (separatorIdx != -1 && parameters.length() - separatorIdx - 1 > 2) {
			throw new HinnakiriNumberDecimalException();
		}
